...
{
    /**
     * يحدِّث/ينشئ التحاليل ويملأ:
     * template, worksheet_template, template_variables(JSON نصي), javascript
     * جدول determinations حسب سكيمتك.
     */
    public function run(): void
    {
        ...
        // الكتالوج: الكود، المجموعة، الترتيب، الاسم، اسم المتغيّر، النوع، الخيارات/الخطوة، الوحدة، والمدى
        $items = [
            // ===== Urine - Macroscopic =====
...
HTML;
        ...
        // JS بسيط لتنبيه الأرقام خارج المدى
...
JS;
        ...
        try {
            foreach ($items as $it) {
                // أنشئ/حدّث السجل الأساسي
                DB::table('determinations')->updateOrInsert(
                    ['nomenclator_id' => $NOMENCLATOR_ID, 'code' => (int) $it['code']],
                    [
                        'name'             => $it['name'],
                        'position'         => (int) $it['pos'],
                ...
                // خريطة المتغيّرات -> قيمة افتراضية نصية (تمنع Array to string)
                $defaultValue = ($it['type'] === 'select')
                    ? (string) ($it['options'][0] ?? '')
                    : '';
                $varsMap = [ $it['var'] => $defaultValue ];
                ...
                DB::table('determinations')
                    ...
                    ->update([
                        ...
                        'template'           => $template,
                        'template_variables' => json_encode($varsMap, JSON_UNESCAPED_UNICODE),
                        'javascript'         => $js,
                        'updated_at'         => $now,
                    ]);
            }
